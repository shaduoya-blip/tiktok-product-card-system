...
2. `changelog.md`：最近更新、规则变化、已补齐能力和历史修复记录。
3. `00_system_control.md`：系统总控、工作顺序、基础边界、竞品资料读取控制和多卖家聚合控制。
4. `01_input_requirements.md`：用户输入、我方产品资料、竞品资料和图片资料的最低要求。
5. `02_output_template.md`：最终商品卡输出结构和模块顺序。
6. `03_core_selling_point_extraction.md`：候选卖点提取、评分和最大卖点确定方法。
7. `04_competitor_listing_absorption.md`：竞品商品卡拆解、吸收、转化和同产品多卖家聚合方法。
8. `06_title_strategy.md`：TikTok Shop US 标题规划规则。
9. `07_search_keyword_strategy.md`：搜索关键词与流量词规划规则。
10. `08_main_image_strategy.md`：主图 6–8 张的顺序、功能和转化逻辑。
11. `09_detail_image_strategy.md`：详情图内容结构、信息层级和展示顺序。
12. `10_product_description_strategy.md`：商品描述写法和信息组织。
13. `11_highlights_strategy.md`：5 点商品亮点写法。
14. `12_us_native_ecommerce_language.md`：美国本土电商英文表达规则。
15. `15_quality_review_checklist.md`：最终质量检查清单。
16. `18_competitor_image_analysis.md`：竞品主图、详情图、SKU 图与产品参考图的图片分析方法。
17. `19_chatgpt_image_prompt_module.md`：ChatGPT 生图提示词生成规则。
18. `20_image_prompt_output_template.md`：单图提示词输出格式。
19. `raw_inputs/竞品资料/产品分组说明.txt`：当前竞品产品组关系。
20. `raw_inputs/研究记录/竞品资料读取报告.md`：当前竞品资料读取与图片分析记录。
21. `knowledge_cards/`：通用知识卡；只在对应任务需要时选择性读取。
22. `examples/`：输入、输出、图片分析和提示词示例；只在需要对齐格式时读取。
